//fetch, export it after finishing 
//promise给的回应
const SERVER_ORIGIN = '';
 
const loginUrl = `${SERVER_ORIGIN}/login`;
 
export const login = (credential) => { //login callback, credential return user info
  return fetch(loginUrl, { //json key
    method: 'POST',
    headers: {
    ...
    return response.json(); //return as json format, return a promise
  })
}
...
const getGameDetails = (gameName) => {
  return fetch(`${getGameDetailsUrl}${gameName}`).then((response) => {//es6
    if (response.status !== 200) {
